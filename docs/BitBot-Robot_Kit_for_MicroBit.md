...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Bitbot1.jpg)
...
!!!Warning
    线跟踪传感器与按钮共用引脚。当 Micro:Bit 启动或重置时，它将检查这 2 个按钮的状态，如果两个按钮都被按下则开始配对，这和您使用的语言有关。在 Bit:Bot 上，这种情况转化为两个线跟踪传感器接收到反馈。您可以在开机前释放按钮，避免上述情况的出现。
...
- 橡胶轮胎的车轮能提供最大抓地力
- 平滑的金属球前轮
- 12 个迷你 neopixel 以 2 列围绕在车身左右。可为任何像素选择任何颜色，当 Bit:Bot 移动时产生酷炫的照明效果
- 2 个数字线跟踪传感器。
...
4.把您的 BBC Micro:Bit 插入金手指

**步骤 1** - 安装前轮

使用 M2 6mm 圆头螺钉和螺母固定前脚轮壳体，然后将脚轮球装入壳体

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step1.jpg)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step2.jpg)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step3.jpg)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step4.jpg)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step5.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step6.jpg)

使用 6mm 的圆头螺钉或 8mm 的沉头螺钉将 12mm 的铜柱安装到主板 (如上图所示)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step7.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step8.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Step9.jpg)

## 硬件概述
---
俯视图

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/bb17a.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/bb15a.jpg)

仰视图可以看到 2 个线跟踪传感器，以及用于 neopixel 扩展和通用扩展连接器的端口。
...
!!!Tip
    示例旨在向人们展示如何使用各种功能。我们不会为您编程 — 授人以鱼不如授人以渔。在下面的例子中，我们将给您提供使用 Bit:Bot 的所有功能的工具，但是师傅领进门，修行靠个人。您需要合理运用以通过代码来实现特定的功能。我们提供了一个完整的线跟踪示例 — 但它是一个非常基本的算法，您可能会对 T-junctions 和交叉点感到困惑，还有它不使用 neopixel。
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download0.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download1.jpg)

如果我们想改变一个电机的速度，使它不是一直在全速运转，我们需要使用 PWM (脉冲宽度调制)。这是一种改变电机获取功率的方法。PWM 的百分比值决定每个周期输出为 ON 的时间。所以 100% 的百分比和上面示例同效。50% 的百分比意味着电机只有一半时间通电，因此速度会变慢。请注意，电机的实际速度与 PWM 的百分比不一样 – 如果 PWM 值太低，电机根本无法转动，并且在某些值下也会出现一些卡顿现象。不过，能够改变速度使得机器人功能更强劲。例如，可以使线跟踪器平稳地跟随线路，而不是左右晃动。
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download2.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download3.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download4.jpg)

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download5.jpg)

### Line Follower Sensors (线跟踪传感器)

Line Follower Sensors (线跟踪传感器) 是数字输入并连接到引脚 **11** (左) 和引脚 **5** (右)。它们与按钮使用的相同的引脚，因此按下按钮将具有与线跟踪相同的效果。这可能会产生意想不到的副作用 – 按下两个按钮时会导致 micro:bit 进入蓝牙配对模式 (取决于安装的软件)。

所以您可以使用正常的按钮输入来读取传感器数据，或者您可以使用 digital_read 命令 (如下所)。如果左边的传感器检测到线，则表示 Bit:Bot 太靠右，所以应该向左移动。如果右侧传感器检测到线，则情况正好相反。下面是 Python 中一些简单的线跟踪代码 (为了清晰起见，实际的电机命令在单独的函数中)
...
在 PXT 环境下，看起来比 Python 更复杂，因为所有的代码都是内联的。您可能需要在循环中添加暂停，具体取决于您的线跟踪轨道。

![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download6.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download7.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download8.jpg)
...
![](https://github.com/SeeedDocument/BitBot-Robot_Kit_for_MicroBit/tree/master/Img/Download9.jpg)

这是每 3 秒打印一次传感器测量到的距离

### Example Micropython Programs (示例 Micropython 程序)

- [Simple PWM motor test](https://4tronix.co.uk/bitbot/PWM_Python.py)
- [Neopixel colour test](https://4tronix.co.uk/bitbot/Pixels_Python.py)
- [Light follower](https://4tronix.co.uk/bitbot/LightFollower.py)
- [Larson Scanner example](https://4tronix.co.uk/bitbot/Scanner.py)
- [Line follower](https://4tronix.co.uk/bitbot/LineFollower2.py)
- [Ultrasonic Obstacle Avoider](https://4tronix.co.uk/bitbot/sonic02.py)
- [POST - Power On Self Test. This test is run on each Bit:Bot before shipping](https://4tronix.co.uk/bitbot/POST.py)
